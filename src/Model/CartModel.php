...
class CartModel
{
    ...
    public function addProduct($productId, $quantity, $productName, $price, $description): void
    {
        // echo $this->cartId;
        ...
        if ($this->cartId) {
            ...
            if ($existing) {
                $stmt = $this->conn->prepare("UPDATE cart_items SET quantity = quantity + ?,price=price+? WHERE cart_id = ? AND product_id = ?");
    ...
    {
        if ($this->cartId) {
            // Fetch cart items with product info using JOIN
